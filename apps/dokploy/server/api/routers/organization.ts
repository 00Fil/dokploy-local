...
export const organizationRouter = createTRPCRouter({
  getStyle: protectedProcedure.query(async ({ ctx }) => {
    const result = await db.query.organization.findFirst({
      where: eq(organization.id, ctx.session.activeOrganizationId),
      columns: { id: true, customCss: true },
    });
    return result
      ? { organizationId: result.id, customCss: result.customCss }
      : null;
  }),
  updateStyle: protectedProcedure
    .input(z.object({ customCss: z.string().max(100000).nullable() }))
